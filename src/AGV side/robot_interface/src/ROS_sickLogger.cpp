...
/* Function to map on the AGV. This function must be run on the AGV after roscore node is running on the network.

Once Lasers have been connected and activated, user can move the AGV to map the environment. The same will be saved to a .2d file "1scans.2d" by default. If user wishes to save to different file, the same must be mentioned while calling the function
*/
...
#ifdef ADEPT_PKG
  #include "Aria.h" 	//Include Aria.h and ArNetworking.h from the library destinations mentioned in the
			//cmakelist file
